...
def LuustoRot():
    ...
    ob = bge.logic.getCurrentController().owner
    try:
        a_rot = a#((a-aMin)/(aMax - aMin))
        b_rot = b#((b-bMin)/(bMax - bMin))
...
def readBluetooth():
    ...
    if (bl_socket != None):
        buffer = buffer + bl_socket.recv(128).decode("utf-8");
        lines = buffer.split('\n')
        while (len(lines) > 1):
        ...
        if (len(line_buffer) > 0):
            ...
            a = data['ShoulderFlexZ_L']/1000.0
            b = data['ShoulderFlexZ_R']/1000.0
